...
## 0.0.4 - ...
**Added:**
- Support Cookie based authentication for secure and simple connections.
- Add `/api/users/invite` endpoint to invite users with email and roles.
- Add `/api/auth/resend-activation-email` endpoint to allow users to resend the activation email.
- Add CLI tool for easier working with the engine [CLI Docs](/docs/bsh-engine/cli).
- Allow creating and installing plugins using the CLI [Plugins Docs](/docs/bsh-engine/packaging/plugins).

**Changed:**
- Reduce Dashboard built chunks sizes for faster loading.
