...
Entities can be configured to be multi-tenant, which means that the records are associated with a tenant. so users can only access data which is associated with their tenant.

To configure an entity to be multi-tenant, you can use the **Multi Tenant** checkbox in the entity form.

<Callout type='info'>
    When a entity is multi-tenant, the engine will add a `tenantId` column to the entity as a primary key.
</Callout>
