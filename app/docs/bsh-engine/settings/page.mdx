...
<Callout type='important'>
  These secrets are currently only used for `BshProviderConfigs` entity. This one is used to store the configuration for [Emails](/docs/bsh-engine/mailing/configurations) and [Storage](/docs/bsh-engine/files/configurations).
</Callout>

### Used Secrets

The engine uses built-in secrets to configure various services (mailing, storage, JWT) via environment variables.<br/>
See [environment secrets](/docs/bsh-engine/environment#secrets) to learn how to pass those secrets to the engine.
