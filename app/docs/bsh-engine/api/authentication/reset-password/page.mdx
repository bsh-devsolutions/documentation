---
title: Reset Password
---

import { Callout } from 'nextra/components';

# Reset Password

The **Reset Password** endpoint allows you to reset your password using a verification code.

```bash
POST /api/auth/reset-password
```

## Request Body

```json
{
  "email": "user@example.com",
  "code": "123456",
  "newPassword": "newpassword123"
}
```

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `email` | string | yes | The email address of the user. |
| `code` | string | yes | The verification code sent to the email address. |
| `newPassword` | string | yes | The new password for the user. |

## Response

```json
{
  "data": [],
  "timestamp": 1234567890,
  "code": 200,
  "status": "OK",
  "meta": {}
}
```

<Callout type='info'>
  If the endpoint succeeds, it will reset the password for the user; then send [Password Changed Email](/docs/bsh-engine/mailing/email-templates#password-changed) to confirm the password reset.
</Callout>
